...
class CalvinVLMDatasetBuilder(CalvinDatasetBuilder):
    ...
    def build_trajectory_representation(self, gripper_centers_world, gripper_widths):
        ...
        for (gripper_center_world, gripper_width) in zip(gripper_centers_world, gripper_widths):
            # round gripper center coords (using tuple.round(self.traj_string_coords_precision) would produce 0.5 instead of 0.500, which *could* be worse for tokenization)
            gripper_center_world_x = f"{gripper_center_world[0]:.{self.traj_string_coords_precision}f}"
            gripper_center_world_y = f"{gripper_center_world[1]:.{self.traj_string_coords_precision}f}"
            gripper_center_world_z = f"{gripper_center_world[2]:.{self.traj_string_coords_precision}f}"
            traj_string_contents.append(f"({gripper_center_world_x}, {gripper_center_world_y}, {gripper_center_world_z})")
    ...
    def _save_trajectory_strings(self, task_all_seqs, task_text_all_seqs, traj_string_all_seqs, start_imgs_all_seqs, dataset_split):
        ...
        assert len(task_all_seqs) == len(traj_string_all_seqs), f"len(task_all_seqs) ({len(task_all_seqs)}) != len(traj_string_all_seqs) ({len(traj_string_all_seqs)})"
        ...
        dataset_entries = []
        for i, (task_seq, task_text_seq, traj_string_seq, start_imgs_seq) in tqdm(enumerate(zip(task_all_seqs, task_text_all_seqs, traj_string_all_seqs, start_imgs_all_seqs)),
            ...
            dataset_entry = {
                "messages": [{
                    "content": get_prompt(task_text_seq),
                    "role": "user"
                },{
    ...
    def build_dataset(self):
        for dataset_split in ["training", "validation"]:
            task_all_seqs, task_text_all_seqs, traj_string_all_seqs, start_imgs_all_seqs, _ = self._build_trajectories(dataset_split)

            self._save_trajectory_strings(task_all_seqs, task_text_all_seqs, traj_string_all_seqs, start_imgs_all_seqs, dataset_split)
