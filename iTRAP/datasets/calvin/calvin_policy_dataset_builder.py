...
class CalvinPolicyDatasetBuilder(CalvinDatasetBuilder):
    ...
    def _embed_and_save_trajectory_imgs(self, task_all_seqs, traj_imgs_all_seqs, dataset_split):
        ...
        auto_vis_lang_ann = {"vision": {"static": [], "gripper": []}, "language": auto_lang_ann["language"], "info": auto_lang_ann["info"]}
        for traj_imgs_seq in tqdm(traj_imgs_all_seqs, total=len(traj_imgs_all_seqs), desc=f"Embedding trajectory images for {dataset_split} split"):
            ...
            # don't embed trajectory images as they're not used for training
            
            auto_vis_lang_ann["vision"]["static"].append(first_static_traj_img)
            auto_vis_lang_ann["vision"]["gripper"].append(first_gripper_traj_img)
        
        auto_vis_lang_ann["vision"]["static"] = np.stack(auto_vis_lang_ann["vision"]["static"])
        auto_vis_lang_ann["vision"]["gripper"] = np.stack(auto_vis_lang_ann["vision"]["gripper"])
